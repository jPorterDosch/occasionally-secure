def main():
  ...
  for line in data:
    ...
    # store lines
      ...
      # if t == 0: print(t,p*3+m,a) 
      tasks[t][p*3+m][a].append(l)
  ...
  for i in range(9):
    for j in range(2):
      for k in range(3):
        ...
        # print(len(tasks[i][j*3+k][0]))
        for line in tasks[i][j*3+k][0]:
          # print(line)
          if "blocks (classes, functions, methods) analyzed." in line: complex_results[i].append(int(line.split(' ')[0]))
